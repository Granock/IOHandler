...
using Boßelwagen.Addons.Core.Switcher.Internal;
...
public class Switcher(IConfigurationService configurationService,
    ...
    private void SwitchWindowCore(SwitchType switchType) {
        ...
        if (!_initialized && !Initialize()) {
            logger.LogWarning("Switcher was not initialized, aborting");
            return;
        }
    ...
    private void InitializeCore() {
        ...
        logger.LogInformation(message: "Switcher found {WindowConfig} window-configs", config.Windows.Count);

        foreach (WindowWrapper windowWrapper in allWindows) {
            WindowConfiguration? windowConfiguration =
                config.Windows.FirstOrDefault(x => x.ProcessName == windowWrapper.ProcessName);
        ...
        _windows = [.. windows.Where(x => x.Enabled).OrderBy(x => x.Configuration.ShowOrder)];
